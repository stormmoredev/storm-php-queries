...
Inserting product
```php
$query->insert('products', ['name' => 'Golden socks', 'price' => 777])->execute();
```
Updating product
```php
$queries->update('products', 'id = ?', 5, ['name' => 'Renamed product'])->execute();
//or
$queries->update('products', ['id' => 5], ['name' => 'Renamed product'])->execute();
```
...
Deleting product
```php
$queries->delete('products', 'id = ?', 5)->execute();
//or
$queries->delete('products', ['id' => 5])->execute();
```
